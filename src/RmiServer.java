...
//        System.out.println("adding client "+user+"to meeting "+nMeeting+" to agenda item "+nAgenda);
    ...
    public boolean inviteUserToMeeting(int nMeeting, String user) throws RemoteException {
        ...
        for (Meeting m : meetings) {
            if (m.getStartDate().after(now) && m.getEndDate().after(now))
                i++;
            if (i == nMeeting) {
                if(m.getResponsibleUser().getUserName().equals(user))
                    return false;
    ...
    public void firstUse() throws RemoteException {
        ...
        addNewMeeting("manel-talk about Jon-wall-jonthemannis-25/10/2014,15:20-Stannis Baratheon,Jon Snow-Ai5,Ai6-360");
